...
from cerebro.engine.deletion import DeletionEngine, DeletionPolicy, DeletionRequest, BatchDeletionResult
...
class CerebroPipeline:
    ...
    def execute_delete_plan(
        ...
        policy = DeletionPolicy.DELETE_PERMANENTLY if plan.mode == "permanent" else DeletionPolicy.MOVE_TO_TRASH
        ...
        try:
            ...
            self._log(f"Deletion engine failed: {e}", level="error")
            raise
...
# PipelineRequest: unified model from domain, re-exported via .models (see __all__)
